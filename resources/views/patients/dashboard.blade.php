...
<script>
	...
	$(".book").click(function(){
		...
		$.ajax({
               ...
               success:function(data) {
                 if(data.success){
					$("#doc_"+doc_id).hide();
					$html = "<div>"+data.msg+"</div><div>Current token:"+data.token.current_token+"<b></b></div><div>Your token number is <b>"+data.token.token_number+"</b> and estimated time is <b>"+data.token.estimated_time+"</b> minute</div><div><button class='refresh_status' id='doc_"+doc_id+"' type='button'>Refresh</button></div>"
	...
	$(document).on("click", ".refresh_status", function () {	
		...
		$.ajax({
               type:'GET',
               url:'/user_dashboard/refresh-status/'+doc_id,
               success:function(data) {
                 if(data.success){					
					$html = "<div>Current token:"+data.token.current_token+"<b></b></div><div>Your token number is <b>"+data.token.token_number+"</b> and estimated time is <b>"+data.token.estimated_time+"</b> minute</div><div><button class='refresh_status' id='doc_"+doc_id+"' type='button'>Refresh</button></div>"
					$(".token_details").html($html);
				 }else{
					 $html = "<div>"+data.msg+"</div>"
					$(".token_details").html($html);
				 }
